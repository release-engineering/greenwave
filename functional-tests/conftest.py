...
def wait_for_listen(port):
    ...
    for attempt in range(50):
        try:
            ...
            time.sleep(0.1)
    raise RuntimeError(f"Gave up waiting for port {port}")
...
def server_subprocess(
    ...
    if source_path is None:
        ...
        if not os.path.isdir(source_path):
            raise RuntimeError(f"{name} source tree {source_path} does not exist")
...
def resultsdb_server(tmpdir_factory):
    ...
    settings_content = f"""
        PORT = 5001
        SQLALCHEMY_DATABASE_URI = 'postgresql+psycopg2:///{dbname}'
        DEBUG = False
...
def waiverdb_server(tmpdir_factory):
    ...
    settings_content = f"""
        AUTH_METHOD = 'dummy'
        DATABASE_URI = 'postgresql+psycopg2:///{dbname}'
        MESSAGE_BUS_PUBLISH = False
        """

    init_db_arguments = [
        "python3",
        os.path.join("waiverdb", "manage.py"),
        "db",
        "upgrade",
    ]
    start_server_arguments = [
...
class TestDataBuilder:
    ...
    def unique_nvr(self, name="glibc", product_version="el7"):
        return f"{name}-1.0-{next(self._counter)}.{product_version}"

    def unique_compose_id(self):
        return f"Fedora-Rawhide-19700101.n.{next(self._counter)}"
