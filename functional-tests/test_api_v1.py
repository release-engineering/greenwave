...
def drop_href(obj):
    if isinstance(obj, dict):
        return {k: drop_href(v) for k, v in obj.items() if k != "href"}
    return obj
...
def test_any_policies_loaded(requests_session, greenwave_server):
    r = requests_session.get(
        greenwave_server + "api/v1.0/policies",
...
def test_about_endpoint_jsonp(requests_session, greenwave_server):
    ...
    assert r.status_code == 200
    expected = f'bac123({{"version":"{__version__}"}});'
...
@pytest.mark.smoke
def test_cannot_make_decision_without_product_version(
...
@pytest.mark.smoke
def test_cannot_make_decision_without_subject_identifier(
    requests_session, greenwave_server
):
    data = {
        "decision_context": "bodhi_update_push_stable",
...
@pytest.mark.smoke
def test_404_for_invalid_product_version(
    ...
    expected = (
        "Found no applicable policies for koji_build subjects "
...
@pytest.mark.smoke
def test_404_for_invalid_decision_context(
    requests_session, greenwave_server, testdatabuilder
):
    nvr = testdatabuilder.unique_nvr()
    data = {
...
def test_invalid_payload(requests_session, greenwave_server):
    ...
    assert (
        "Failed to decode JSON object" in message
        or "sent a request that this server could not understand" in message
    )


def test_make_a_decision_on_passed_result(
    requests_session, greenwave_server, testdatabuilder
):
    nvr = testdatabuilder.unique_nvr()
...
def test_make_a_decision_with_verbose_flag(
    requests_session, greenwave_server, testdatabuilder
):
    nvr = testdatabuilder.unique_nvr()
    results = []
...
def test_make_a_decision_with_verbose_flag_and_multiple_nvrs_with_results(
    ...
    data = {
        "decision_context": "bodhi_update_push_stable",
        "product_version": "fedora-26",
        "subject": [{"type": "koji_build", "item": nvr} for nvr in build_nvrs],
        "verbose": True,
    }
    ...
    assert len(res_data["results"]) == len(TASKTRON_RELEASE_CRITICAL_TASKS) * len(
        build_nvrs
    )
    assert drop_hrefs(res_data["results"]) == drop_hrefs(list(reversed(results)))
...
def test_make_a_decision_with_verbose_flag_and_multiple_nvrs_with_waivers(
    ...
    assert len(res_data["waivers"]) == len(TASKTRON_RELEASE_CRITICAL_TASKS) * len(
        build_nvrs
    )
...
def test_make_a_decision_on_failed_result_with_waiver(
    ...
    testdatabuilder.create_result(
        item=nvr, testcase_name=TASKTRON_RELEASE_CRITICAL_TASKS[0], outcome="FAILED"
    )
...
def test_make_a_decision_on_failed_result(
    requests_session, greenwave_server, testdatabuilder
):
    nvr = testdatabuilder.unique_nvr()
...
def test_make_a_decision_on_no_results(
    requests_session, greenwave_server, testdatabuilder
):
    nvr = testdatabuilder.unique_nvr()
    data = {
...
def test_make_a_decision_on_redhat_cont_image(
    ...
    data = {
        ...
        "rules": [
            {"type": "PassingTestCaseRule", "test_case_name": "test.testcase1"},
            {"type": "PassingTestCaseRule", "test_case_name": "test.testcase2"},
        ],
    }
    r = requests_session.post(greenwave_server + "api/v1.0/decision", json=data)
    assert r.status_code == 200
...
def test_multiple_results_in_a_subject(
    ...
    nvr = testdatabuilder.unique_nvr()
    testdatabuilder.create_result(
        item=nvr, testcase_name="dist.abicheck", outcome="PASSED"
    )
...
def test_ignore_result(requests_session, greenwave_server, testdatabuilder):
    ...
    result = testdatabuilder.create_result(
        item=nvr, testcase_name=TASKTRON_RELEASE_CRITICAL_TASKS[0], outcome="PASSED"
    )
    data = {
        "decision_context": "bodhi_update_push_stable",
        "product_version": "fedora-26",
    ...
    data.update({"ignore_result": [result["id"]]})
    r = requests_session.post(greenwave_server + "api/v1.0/decision", json=data)
    expected_unsatisfied_requirements = [
        {
            "item": {"item": nvr, "type": "koji_build"},
...
def test_ignore_waiver(requests_session, greenwave_server, testdatabuilder):
    ...
    assert res_data["policies_satisfied"] is True
    # Ignore the waiver
    data.update({"ignore_waiver": [waiver["id"]]})
    r_ = requests_session.post(greenwave_server + "api/v1.0/decision", json=data)
    assert r_.status_code == 200
    res_data = r_.json()
    expected_unsatisfied_requirements = [
...
def test_distgit_server(requests_session, distgit_server, tmpdir):
    ...
    """
    r_ = requests_session.head(
        distgit_server, headers={"Content-Type": "application/json"}, timeout=60
    )
    assert r_.status_code == 200
...
def test_cached_false_positive(requests_session, greenwave_server, testdatabuilder):
    ...
    testdatabuilder.create_result(
        item=nvr, testcase_name=TASKTRON_RELEASE_CRITICAL_TASKS[-1], outcome="FAILED"
    )
    r = requests_session.post(greenwave_server + "api/v1.0/decision", json=data)
    assert r.status_code == 200
    res_data = r.json()
...
def test_excluded_packages(requests_session, greenwave_server, testdatabuilder):
    ...
    testdatabuilder.create_koji_build_result(
        nvr=nvr,
        testcase_name="osci.brew-build.tier0.functional",
        outcome="FAILED",
        type_="brew-build",
    )
    data = {
...
def test_make_a_decision_about_brew_build(
    requests_session, greenwave_server, testdatabuilder
):
    ...
    assert (
        res_data["summary"] == "All required tests (1 total) have passed or been waived"
    )
...
def test_validate_gating_yaml_missing_decision_context(
    requests_session, greenwave_server
):
    gating_yaml = dedent("""
        --- !Policy
    ...
    assert result.json().get("message") == (
        "Greenwave could not find a parent policy(ies) for "
...
def test_make_a_decision_about_compose_all_variants_architectures(
    ...
    failed_results = testdatabuilder.create_rtt_compose_result(
        compose_id=compose_id, outcome="FAILED", **variant1
    )

    testdatabuilder.create_rtt_compose_result(
        compose_id=compose_id, outcome="PASSED", **variant2
    )
...
def test_make_a_decision_about_compose_latest_variants_architectures(
    ...
    testdatabuilder.create_rtt_compose_result(
        compose_id=compose_id, outcome="FAILED", variant="BaseOS", architecture="ppc64"
    )

    testdatabuilder.create_rtt_compose_result(
        compose_id=compose_id, outcome="PASSED", variant="BaseOS", architecture="ppc64"
    )
...
def test_make_a_decision_about_compose_new_variants_architectures(
    ...
    for arch, outcome in [
        ...
        testdatabuilder.create_rtt_compose_result(
            compose_id=compose_id, variant="BaseOS", architecture=arch, outcome=outcome
        )
...
def test_make_a_decision_for_bodhi_with_verbose_flag(
    ...
    data = {
        ...
        "subject": [{"type": "koji_build", "item": nvr} for nvr in nvrs]
...
def test_verbose_retrieve_latest_results(
    requests_session, greenwave_server, testdatabuilder
):
    nvr = testdatabuilder.unique_nvr()
    for outcome in ["FAILED", "PASSED"]:
        for testcase_name in TASKTRON_RELEASE_CRITICAL_TASKS:
            testdatabuilder.create_result(
                item=nvr, testcase_name=testcase_name, outcome=outcome
            )
    data = {
        "decision_context": "bodhi_update_push_stable",
        "product_version": "fedora-26",
...
def test_verbose_retrieve_latest_results_scenario(
    requests_session, greenwave_server, testdatabuilder
):
    nvr = testdatabuilder.unique_nvr()
    results = []
...
def test_api_with_when(requests_session, greenwave_server, testdatabuilder):
    ...
    results.append(
        testdatabuilder.create_result(
            item=nvr, testcase_name=TASKTRON_RELEASE_CRITICAL_TASKS[1], outcome="PASSED"
        )
    )
    results.append(
        testdatabuilder.create_result(
            item=nvr, testcase_name=TASKTRON_RELEASE_CRITICAL_TASKS[2], outcome="FAILED"
        )
    )
    data = {
        "decision_context": "bodhi_update_push_stable",
...
def test_cannot_make_decision_without_required_rule_type(
    ...
    data = {
        ...
        "subject_identifier": "FEDORA-2018-ec7cb4d5eb",
        "rules": [
            {"typo": "PassingTestCaseRule", "test_case_name": "dist.abicheck"},
            {"type": "PassingTestCaseRule", "test_case_name": "dist.rpmdeplint"},
        ],
    }
    r = requests_session.post(greenwave_server + "api/v1.0/decision", json=data)
    assert r.status_code == 400
...
def test_cannot_make_decision_without_required_rule_testcase_name(
    ...
    data = {
        ...
        "subject_identifier": "FEDORA-2018-ec7cb4d5eb",
        "rules": [
            {"type": "PassingTestCaseRule", "test_case_name": "dist.abicheck"},
            {"type": "PassingTestCaseRule"},
        ],
    }
    r = requests_session.post(greenwave_server + "api/v1.0/decision", json=data)
...
def test_make_a_decision_with_verbose_flag_on_demand_policy(
    ...
    data = {
        ...
        "rules": [
            {"type": "PassingTestCaseRule", "test_case_name": "dist.abicheck"},
            {"type": "PassingTestCaseRule", "test_case_name": "dist.rpmdeplint"},
            {"type": "PassingTestCaseRule", "test_case_name": "dist.upgradepath"},
        ],
    }
    r = requests_session.post(greenwave_server + "api/v1.0/decision", json=data)
    assert r.status_code == 200
...
def test_make_a_decision_all_scenarios_waived(
    ...
    assert res_data["policies_satisfied"] is True
    assert (
        res_data["summary"] == "All required tests (2 total) have passed or been waived"
    )
