...
from greenwave.config import Config, TestingConfig
...
from greenwave.resources import KojiScmUrlParseError, ResultsRetriever
...
class DummyResultsRetriever(ResultsRetriever):
    def __init__(self, subject=None, testcase=None, outcome="PASSED", when=""):
        super().__init__(ignore_ids=[], when=when, url="")
    ...
    def _retrieve_data(self, params):
        ...
        if (
            self.subject
            and (
                params.get("item") == self.subject.identifier
                or params.get("nvr") == self.subject.identifier
            )
            and ("type" not in params or self.subject.type in params["type"].split(","))
            and (
                params.get("testcases") is None
                or params.get("testcases") == self.testcase
            )
        ):
            return [
                {
...
def test_load_policies():
    ...
    assert any(
        policy.id == "taskotron_release_critical_tasks"
        for policy in app.config["policies"]
    )
    assert any(
        policy.decision_context == "bodhi_update_push_stable"
        for policy in app.config["policies"]
    )
    assert any(
        policy.all_decision_contexts == ["bodhi_update_push_stable"]
        for policy in app.config["policies"]
    )
    assert any(
        getattr(rule, "test_case_name", None) == "dist.rpmdeplint"
        for policy in app.config["policies"]
...
def test_remote_rule_policy_redhat_module(tmpdir, namespace):
    ...
    with mock.patch("greenwave.resources.retrieve_scm_from_koji") as scm:
        ...
        with mock.patch("greenwave.resources.retrieve_yaml_remote_rule") as f:
            ...
            # Ensure that presence of a result is success.
            results = DummyResultsRetriever(
                subject, "baseos-ci.redhat-module.tier0.functional"
            )
...
def test_redhat_container_image_subject_type():
    ...
    retriever = ResultsRetriever(ignore_ids=[], when=cur_time, url=rdb_url)
...
def test_parse_policies_remote_missing_id_is_ok():
    policies = RemotePolicy.safe_load_all(
        dedent("""
        --- !Policy
        product_versions: [fedora-rawhide]
        decision_context: test
...
def test_on_demand_policy_match(two_rules, koji_proxy):
    ...
    results = DummyResultsRetriever(subject, "fake.testcase.tier0.validation", "PASSED")
